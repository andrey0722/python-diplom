...
async def verify_email(session: httpx.AsyncClient, email: str) -> None:
    ...
    response = await session.post(
        f'{API_ROOT}/user/register/verify', data=data
    )
    fail_if_error(response)
    json: dict[str, Any] = response.json()
    ...
    response = await session.post(
        f'{API_ROOT}/user/register/confirm', data=data
    )
    fail_if_error(response)
...
async def login_user(
    ...
    response = await session.post(f'{API_ROOT}/user/login', data=data)
    fail_if_error(response)
    json: dict[str, Any] = response.json()
    return json['token']
...
async def get_contacts(
    ...
    response = await session.get(f'{API_ROOT}/user/contact', headers=headers)
    fail_if_error(response)
...
async def create_contact(
    ...
    Returns:
        Contact: Created contact payload.
    """
...
async def get_products(
    ...
    response = await session.get(f'{API_ROOT}/products', headers=headers)
    fail_if_error(response)
    return response.json()
...
async def get_basket(
    ...
    """
    headers = auth_header(user_token)
    response = await session.get(f'{API_ROOT}/basket', headers=headers)
...
async def add_to_basket(
    ...
    headers = auth_header(user_token)
    response = await session.post(
        f'{API_ROOT}/basket', data=data, headers=headers
    )
    fail_if_error(response)
    return response.json()


async def edit_basket(
    session: httpx.AsyncClient,
    user_token: str,
    items: OrderItemsData,
) -> OrderData:
...
async def cancel_order(
    ...
    response = await session.delete(
        f'{API_ROOT}/order/{order_id}', headers=headers
    )
    fail_if_error(response)


async def update_shop_pricing(
    session: httpx.AsyncClient,
    user_token: str,
    url: str,
) -> None:
...
async def login(session: httpx.AsyncClient, user: UserData) -> str:
    ...
    Returns:
        str: API authentication token.
    """
    return await login_user(session, user.email, user.password)


async def create_user(session: httpx.AsyncClient, user: UserData) -> str:
...
async def create_shop(
    ...
    """
    token = await create_user(session, shop)
    await update_shop_pricing(session, token, shop_url)
    return token


async def create_shop_template(
    session: httpx.AsyncClient,
    name: object,
    password: str,
    file: Path,
    ...
    async with asyncio.TaskGroup() as tg:
        for file in SHOP_DATA_DIR.iterdir():
            ...
            if name := match and match.group(1):
                task = tg.create_task(
                    create_shop_template(session, name, USER_PASSWORD, file),
                )
                tasks.append(task)
    return [task.result() for task in tasks]


async def create_default_contacts(
    session: httpx.AsyncClient,
    user_token: str,
) -> Contacts:
    ...
    Returns:
        Contacts: Created contact payloads.
    """
    if CONTACTS_COUNT < 1:
        return []
    ...
    async with asyncio.TaskGroup() as tg:
        tasks = [
            tg.create_task(create_contact(session, user_token, contact))
...
async def create_default_users(
    ...
    async with asyncio.TaskGroup() as tg:
        tasks = [
            tg.create_task(create_user_with_contacts(session, user))
            for user in users
        ]
    return [task.result() for task in tasks]


async def empty_basket(session: httpx.AsyncClient, user_token: str) -> None:
    ...
    Args:
        ...
        user_token (str): API authentication token.
    """
    if basket := await get_basket(session, user_token):
        items = [item['id'] for item in basket['items']]
        await delete_from_basket(session, user_token, items)
...
async def fill_basket(
    ...
    Returns:
        OrderData: Updated basket payload.
    """
    offers = await get_products(session, user_token)
    items = select_items_for_order(offers)
    basket = await add_to_basket(session, user_token, items)
    items = edit_basket_items(offers, basket)
    return await edit_basket(session, user_token, items)
